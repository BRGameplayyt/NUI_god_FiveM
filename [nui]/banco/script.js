...
$(document).ready(function(){
    ...
    window.addEventListener('message', function(event){
        ...
        if(item.updateBalance == true) {
            $('.currentBalance').html(Number(event.data.balance).toLocaleString('pt-BR'));
            $('.username').html(event.data.player);
        }
        // Trigger Add Balance Popup
        if(item.addBalance == true) {
            var element = $('<p id="add-balance"><span class="pre">+</span><span class="green"> R$ </span>' + Number(event.data.balance).toLocaleString('pt-BR') +'</p>');
        ...
        if(item.removeBalance == true) {
            var element = $('<p id="add-balance"><span class="pre">-</span><span class="red"> R$ </span>' + Number(event.data.balance).toLocaleString('pt-BR') +'</p>');
            $(".transaction").append(element);
            setTimeout(function(){
                $(element).fadeOut(600, function() { $(this).remove(); })
            }, 1000)
    ...
    document.onkeyup = function (data) {
        if (data.which == 27 ) {
            $.post('http://banco/close');
        }
    };
    // Handle Button Presses
    $(".btnWithdraw").click(function(){
        $.post('http://banco/withdraw');
    });
    $(".btnDeposit").click(function(){
        $.post('http://banco/deposit');
    });
    $(".btnTransfer").click(function(){
        $.post('http://banco/transfer');
    });
    $(".btnBalance").click(function(){
        $.post('http://banco/balance');
    });
    $('.btnQuick').click($.throttle( 2000, true, function(e){
        $.post('http://banco/quickCash');
    }));
    ...
    $("#withdraw-form").submit(function(e) {
        e.preventDefault();
        $.post('http://banco/withdrawSubmit', JSON.stringify({
    ...
    $("#deposit-form").submit(function(e) {
        e.preventDefault();
        $.post('http://banco/depositSubmit', JSON.stringify({
    ...
    $("#transfer-form").submit(function(e) {
        e.preventDefault();
        $.post('http://banco/transferSubmit', JSON.stringify({
